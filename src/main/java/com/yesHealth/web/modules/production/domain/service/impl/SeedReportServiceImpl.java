...
//		o3.setColIndex(14);
	...
	private SeedReport mapToSeedReport(Map<String, String> map, FileUploadRecords fileUploadRecords) {
		...
		seedReport.setSeqNo(map.get("序"));

		String manuNo = map.get("工單號碼");
		if (manuNo != null) {
			ProductSchedule ps = planRepository.findByManuNo(manuNo).get(0);
			seedReport.setPs(ps);
		}

		seedReport.setBoardCount(Long.valueOf(map.get("boardCount")));
		seedReport.setBoardPiece(Long.valueOf(map.get("boardPiece")));

		// 使用 LocalDate 代替 Date
		seedReport.setWorkDate(LocalDate.parse(map.get("workDate"))); // 假設日期格式為 "yyyy-MM-dd"

		seedReport.setWorkMan(Long.valueOf(map.get("workMan")));
		seedReport.setWorkTimeStart(LocalDateTime.parse(map.get("workTimeStart"))); // 假設格式為 "yyyy-MM-dd'T'HH:mm:ss"
		seedReport.setWorkTimeEnd(LocalDateTime.parse(map.get("workTimeEnd")));

		seedReport.setGramBeforeUse(Integer.valueOf(map.get("gramBeforeUse")));
		seedReport.setGramAfterUse(Integer.valueOf(map.get("gramAfterUse")));
		seedReport.setCountPerHole(Integer.valueOf(map.get("countPerHole")));
		seedReport.setEstWorkTime(Double.valueOf(map.get("estWorkTime")));
		seedReport.setRemark(map.get("remark"));
